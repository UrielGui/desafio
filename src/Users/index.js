		...
		<Styled.Container>
			<Styled.Options>
				<Styled.Form ref={formRef}>
					...
					/>
					<Styled.FormButton type='button' background='#107b71' onClick={resetForm} disabled={!inputRef.current?.value}>
						Limpar
					</Styled.FormButton>
				</Styled.Form>
				...
				{users
					.filter((val) => {
						if (search === '' || search === null) {
							return val;
						} else if (val.name.first?.toLowerCase().includes(search.toLocaleLowerCase())) {
							return val;
						} else {
							return null;
						}
